...
public func desync<Value>(task: @escaping @Sendable () async throws -> Value, callback: @escaping @Sendable (Result<Value, Error>) -> Void) {
    Task {
        do {
...
public extension Task where Failure == any Error {
    ...
    ///   - task:     The task to run on a separate thread
    ///   - callback: Passed the return value of `task` after it completes, on the same thread upon which `task` was run
    static func desync(_ task: @escaping @Sendable () async throws -> Success, callback: @escaping @Sendable (Result<Success, Failure>) -> Void) {
